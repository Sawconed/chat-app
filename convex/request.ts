...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.email === identity.email) throw new ConvexError("Cannot send request to yourself.");

    const currentUser = await getUserByClerkId({ ctx, clerkId: identity.subject });

    if (!currentUser) throw new ConvexError("Sender not found.");
    ...
    if (requestAlreadyReceived) throw new ConvexError("This user has already sent you a friend request.");
